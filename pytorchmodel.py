...
full_dataset = torchvision.datasets.ImageFolder(r"C:\Users\Jonah\PycharmProjects\TumorClassifier\Data", transform=transform)
split_ratio = 0.75
...
for epoch in range(num_epochs):
    ...
    fitting_factor = (train_accuracy/test_accuracy) - 1
    # Print epoch statistics
    print(f"Epoch {epoch + 1}/{num_epochs}: "
          f"Train Loss: {train_loss:.4f}, Train Acc: {train_accuracy:.2f}%, "
          f"Test Loss: {test_loss:.4f}, Test Acc: {test_accuracy:.2f}% "
          f"Fitting Factor: {fitting_factor:.2f}")
    if (test_accuracy > 96):
      print("Accuracy threshold reached. Ending training cycle.")
      break


torch.save(model.state_dict(), "tumor_cnn_sgd.pt")
